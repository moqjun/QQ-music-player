...
@interface PlayerToolBar()<UIGestureRecognizerDelegate>
...
-(void)awakeFromNib{
    ...
    [self.bgImageView addGestureRecognizer:tapGest];
...
-(MusicTool *)mTool{
    if (!_mTool) {
        _mTool = [MusicTool sharedMusicTool];
    }
...
-(void)setPlayingMusic:(Music *)playingMusic{
    ...
    //初始化左边Label的总时间
    self.totalTimeLabel.text = [NSString getMinuteSecondWithSecond:[MusicTool sharedMusicTool].player.duration];
...
-(void) TapView:(UITapGestureRecognizer *) tapGest
{
    ...
    if (point.x<=width)
    {
        if ([self.delegate respondsToSelector:@selector(doSwitchToFullScreenPlayerController:)]) {
            [self.delegate doSwitchToFullScreenPlayerController:self];
        }
    }
}
